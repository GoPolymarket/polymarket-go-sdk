...
- **`pkg/execution`**: Unified execution contract (`Place`/`Cancel`/`Query`/`Replay`), CLOB adapter, and normalized lifecycle model (`created`/`accepted`/`partial`/`filled`/`canceled`/`rejected`).
